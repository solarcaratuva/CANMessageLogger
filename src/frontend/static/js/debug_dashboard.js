...
// Persist selected tables across page navigations using localStorage
const STORAGE_KEY_SELECTED = 'debug:selectedTables';
const DEFAULT_SELECTED = ["MotorCommands", "DashboardCommands"];
...
let checkedTables = loadSelectedFromStorage() || DEFAULT_SELECTED;
const tableMessages = {};
...
function populateCheckboxes(tableNames) {
    ...
    form.innerHTML = '';
    // On repopulate, prefer persisted selection; otherwise use defaults
    const persisted = loadSelectedFromStorage();
    checkedTables = persisted && persisted.length ? persisted : DEFAULT_SELECTED;
    ...
    // After creating checkboxes, apply visibility based on current selection
...
function loadTableData() {
    checkedTables = Array.from(document.querySelectorAll('input[name="tableName"]:checked'))
        .map(cb => cb.value);
    saveSelectedToStorage(checkedTables);
    updateVisibleTables(checkedTables);
}
        ...
        const messageContent = Object.entries(msg.data)
            .map(([k, v]) => {
                if (typeof v === 'object' && v !== null && 'value' in v && 'unit' in v) {
                    if (v.unit && v.unit !== null && v.unit !== "null" && v.unit !== "") {
